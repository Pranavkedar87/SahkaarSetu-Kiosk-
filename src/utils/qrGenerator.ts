 ...
 * Standard ISO/IEC 18004 compatible QR matrix and SVG generator.
 * Zero external dependencies, 100% React 19 compatible.
...
const QRErrorCorrectLevel: Record<QrErrorCorrectionLevel, number> = {
  L: 1,
  M: 0,
  Q: 3,
  H: 2,
...
class QRRSBlock {
  ...
  static RS_BLOCK_TABLE: number[][] = [
    ...
    // 11
    [4, 81, 57], [1, 50, 28, 4, 51, 29], [4, 42, 24, 4, 43, 25], [3, 36, 12, 8, 37, 13],
    // 12
    [2, 92, 72, 2, 93, 73], [6, 48, 26, 2, 49, 27], [4, 46, 20, 6, 47, 21], [7, 42, 14, 4, 43, 15],
    // 13
    [4, 107, 85], [8, 59, 37, 1, 60, 38], [8, 44, 20, 4, 45, 21], [12, 33, 11, 4, 34, 12],
    // 14
    [3, 115, 91, 1, 116, 92], [4, 64, 40, 5, 65, 41], [11, 46, 16, 5, 47, 17], [11, 36, 12, 5, 37, 13],
  ];

  static getRSBlocks(typeNumber: number, errorCorrectLevel: number): QRRSBlock[] {
    const qrRSBlock = QRRSBlock.getRsBlockTable(typeNumber, errorCorrectLevel);
    if (!qrRSBlock) throw new Error(`bad rs block @ typeNumber:${typeNumber}/errorCorrectLevel:${errorCorrectLevel}`);
...
class QRCodeModel {
  ...
  make(): void {
    if (this.typeNumber < 1) {
      let typeNumber = 1;
      const maxType = QRRSBlock.RS_BLOCK_TABLE.length / 4;
      for (typeNumber = 1; typeNumber <= maxType; typeNumber++) {
      ...
      if (typeNumber > maxType) {
        throw new Error("Text too large for QR generator");
      }
      this.typeNumber = typeNumber;
    }
...
export function generateQrMatrix(text: string, level: QrErrorCorrectionLevel = "M"): boolean[][] {
  const qr = new QRCodeModel(0, QRErrorCorrectLevel[level]);
  qr.addData(text);
  qr.make();
...
export function generateQrSvg(text: string, options: QrSvgOptions = {}): string {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${size} ${size}" width="${size}" height="${size}" role="img" aria-label="QR Code for ${text.replace(/"/g, "&quot;")}">
